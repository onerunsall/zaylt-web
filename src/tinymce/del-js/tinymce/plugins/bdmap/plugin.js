tinymce.PluginManager.add('bdmap', function(editor, url) {
	...
	var bdmap_width = function (editor) {
		return editor.getParam('bdmap_width', '100%');
    };
    var bdmap_height = function (editor) {
		return editor.getParam('bdmap_height', 300);
    };
	window.tinymceLng='';
	window.tinymceLat='';
	var openDialog = function() {
		return editor.windowManager.openUrl({
			...
			onAction: function (api, details) {
				switch (details.name) {
					case 'save':
						html='<iframe src="'+baseURL+'/plugins/bdmap/bd.html?center='+tinymceLng+'%2C'+tinymceLat+'&zoom=14&width='+(bdmap_width(editor)-2)+'&height='+(bdmap_height(editor)-2)+'" frameborder="0" style="width:'+bdmap_width(editor)+';height:'+bdmap_height(editor)+'px;">';
